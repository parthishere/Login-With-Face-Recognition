...
class ThreadManager(models.Manager):
    ...
    def get_or_create(self, first_person, second_person):
        lookup = (Q(first_person=first_person) & Q(second_person=second_person)) | (
            Q(first_person=second_person) & Q(second_person=first_person))
        qs = self.get_queryset().filter(lookup).distinct()
        print(qs)
...
class Thread(models.Model):
    first_person = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE,
