...
def after_setting_allow_attendance_to_true(teacher_username, lecture_id):
    ...
    if lecture_obj.allow_recognize:
        ...
        lecture_obj.save()
        print(
            f"changed allow attendance to false of lecture {lecture_obj.lecture_name}")
    else:
...
def add_user_to_accepted_user_session(user_pk, teacher_username, session_pk):
    ...
    teacher = UserProfile.objects.select_realted("user").prefetch_related(
        "change_website_objects").get(user__username=teacher_username)
    session = teacher.change_website_objects.prefetch_related(
        "requested_user").get(pk=session_pk)
