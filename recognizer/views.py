...
def set_name(teacher, lecture_name):
    lecture_obj = SessionAttendanceModel.objects.filter(
        lecture__lecture_name=lecture_name, teacher__user__username=teacher).count()
...
def enable_disable_session_view(request):
    ...
    if request.method == 'POST':
        ...
        if all_in_one_user.is_staff or all_in_one_user.is_teacher:
            ...
            lecture_obj = all_in_one_user.user_profile.lectures.get(
                pk=lecture_id)
            print(lecture_obj)
...
def home_view(request):
    ...
    if request.method == 'POST' and lecture_details_form.is_valid():
        ...
        ifile = request.FILES.get('image_file').read()
...
def accept_user_from_session(request):
    ...
    if request.POST:
        ...
        add_user_to_accepted_user_session(
            user_pk, request.user.username, session_pk)
...
def reject_request_to_session_view(request):
    ...
    if request.POST:
        ...
        user_pk = request.POST['user_pk']
        session_pk = request.POST['session_pk']
        remove_user_from_requested_user_session(
            user_pk, request.user.username, session_pk)
    return redirect(next_)
...
def remove_from_atendees_view(request):
    ...
    if request.POST:
        ...
        user_pk = request.POST['user_pk']
        session_pk = request.POST['session_pk']
        remove_user_from_atendees_session(
            user_pk, request.user.username, session_pk)
    return redirect(next_)
    ...
    if request.POST:
        if login_form.is_valid():
            ...
            if user is not None:
                ...
                request.session['user_pk'] = UserProfile.objects.get(
                    user=user).pk
...
def profile_view(request, pk=None):
    ...
    login_instance = None
    try:
        instance = UserProfile.objects.select_related(
            "user").prefetch_related("user__attandence").get(pk=pk)
    except Exception as e:
        print(e)
        pass
...
def lecture_details(request, pk=None):
    ...
    for x in login:
        if f"{x.lecture.lecture_name}/{x.teacher.user.username}" in x_lables:
            ...
            x_lables.append(
                f"{x.lecture.lecture_name}/{x.teacher.user.username}")
    for i in x_lables:
        i = i.split('/')
        lecture = i[0]
        teacher = i[1]
        y_lables.append(user_profile.user.attandence.filter(
            lecture__lecture_name=lecture, teacher__user__username=teacher).count())
...
def update_profile_view(request, pk=None):
    ...
    if (not instance.user.is_updated) or instance.user.is_teacher:
        edit_form = FirstTimeUserProfileForm(
            request.POST or None, request.FILES or None, instance=instance)
    else:
        edit_form = SecondTimeUserProfileForm(
            request.POST or None, request.FILES or None, instance=instance)
    context = {
        'form': edit_form,
    }
...
def update_profile_image_view(request, pk=None):
    try:
        ...
        instance = None
    edit_form = UserProfileImageForm(
        request.POST or None, request.FILES or None, instance=instance)
    context = {
        'form': edit_form,
    }
...
def login_with_face(request):
    ...
    if request.method == 'POST':
        ...
        names, known_lables, login_proceed = Recognizer(
            details, username=user.user.username, unique_id=user.unique_id)
        ...
        if str(request.user.username + user.unique_id) in names:
            ...
            messages.error(
                request, 'stfu b** get your ass out of my website..')
            return redirect('recognizer:home')
